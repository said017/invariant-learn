...
/// @dev Helper that holds underlying tokens and can deposit/withdraw on behalf of Echidna callers.
///      Needed because the vault cannot safeTransferFrom itself without double-counting.
contract EchidnaVaultHelper {
...
contract Echidna_SimpleERC4626 is SimpleERC4626 {
        ...
        )
    {
        ...
        helper = new EchidnaVaultHelper(this, mockAsset);
    }
    ...
    // ─── Handler functions ───────────────────────────────────────
    //
    // The helper contract holds underlying tokens, approves the vault,
    // and calls deposit/mint. This avoids the vault transferring from
    // itself to itself (which wouldn't increase the underlying balance).
    ...
    function handler_withdraw(uint256 assets) external {
        ...
        // Grant the helper allowance to pull shares from msg.sender
    ...
    function echidna_balance_sum_eq_total_supply() public view returns (bool) {
        uint256 sum = 0;
    ...
    function echidna_zero_supply_implies_zero_assets() public view returns (bool) {
        if (totalSupply == 0) {
    ...
    function echidna_no_free_value_on_roundtrip() public view returns (bool) {
